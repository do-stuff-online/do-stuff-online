DSO.defineMode('hq9+', async (code,input,args,output,debug) => {
    ...
    var rot13 = (val) => val.replace(/[a-zA-Z]/g,x=>String.fromCharCode((x.charCodeAt()-(x>='a'?97:65)+13)%26+(x>='a'?97:65)))
    for(var char of code){
        if(char == '9') output(bottles);
        if(char == 'Q') output(code);
        if(char == 'H') output('Hello, world!');
        if(char == 'C') output(input)
        if(char == 'I') DSO.activeMode.run(input,input,args,output,debug)
